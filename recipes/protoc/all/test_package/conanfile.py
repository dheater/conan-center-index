...
class TestPackageConan(ConanFile):
    ...
    def _protoc_available(self):
        return not self.options["protoc"].lite and not tools.cross_building(self.settings)

    def build(self):
        # Build without protoc
        os.mkdir("without_protoc")
        shutil.copy(os.path.join(self.source_folder, "addressbook.{}.pb.h".format(self.deps_cpp_info["protoc"].version)),
                    os.path.join("without_protoc", "addressbook.pb.h"))
        shutil.copy(os.path.join(self.source_folder, "addressbook.{}.pb.cc".format(self.deps_cpp_info["protoc"].version)),
